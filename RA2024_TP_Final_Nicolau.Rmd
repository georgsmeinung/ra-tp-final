...
references:
  ...
  - id: cardenas-2022
    author: "Cardenas, Christian & Lozano, David & Marquez, Cristian & Torres, Edilberto & Delgado-Correal, Camilo" 
    title: "Optimización de un sistema difuso para la detección automática de tránsitos planetarios en curvas de luz de estrellas individuales"
    journal: "Ciencia en Desarrollo"
    year: 2022 
    volume: 1
    number: 10
    pages: "19-35" 
    url: "http://dx.doi.org/10.19053/01217488.v1.n2E.2022.15136"
  - id: cermak-2024
  ...
  - id: hadrien-2025
    author: "Hadrien Cambazard, Nicolas Catusse, Antoine Chomez, Anne-Marie Lagrange"
    ...
    number: 2
    pages: "1610–1624"
    url: "https://doi.org/10.1093/mnras/stae2657"
  ...
  - id: lee-2013
    ...
    url: "https://citeseerx.ist.psu.edu/document?repid=rep1&type=pdf&doi=798d9840d2439a0e5d47bcf5d164aa46d5e7dc26"
  - id: malik-2021
    author: "Malik, A., Moster, B. P., & Obermeier, C."
  ...
  - id: tukey-1977
    ...
    volume: 2
  - id: ventaka-2023
    author: "Venkata, G & Jahnavi, M & Ch, Venkata & Suneetha, Muvva"
    title: "Exoplanet Detection Using Feature Engineering with Ensemble Learning"
    year: 2023
    url: "http://dx.doi.org/10.1109/ICPCSN58827.2023.00025"
  ...
  - id: wu-2023
    author: "Wu, Dong-Hong"
    title: "The possibility of detecting our solar system through astrometry"
    year: 2023
    url: "https://arxiv.org/abs/2309.11729"
  - id: zhou-2004
    author: "Zhou, D., Bousquet, O., Lal, T. N., Weston, J., & Schölkopf, B."
...
Para detectar estos mundos, los astrónomos emplean diversas técnicas, muchas de ellas llevadas al límite de la tecnología disponible en los observatorios espaciales [@nardi-2024]: astrometría o detección a través del movimiento estelar [@wu-2023], velocidades radiales o detección a través del efecto Doppler [@marin-2018], método del tránsito o detección a través de la sombra planetaria [@alonso-sobrino-2006], microlente gravitacional o detección a través de la curvatura del espacio-tiempo [@marin-2011], observación directa o fotografía en medio del brillo estelar [@nasa-2022] y detección por cómo afectan la rotación de púlsares [@wolszczan-frail-1992].
...
Todos los archivos necesarios para este trabajo se encuentran en el repositorio de GitHub del autor [@nicolau-2025] y se pueden descargar desde allí para reproducir este análisis, incluyendo el dataset **`keplerexoplanets.csv`** que a continuación se carga.
...
```{r show_filtered_data_structure}
library(dplyr)
library(knitr)
...
```{r eda_filtered_data, message=FALSE, warning=FALSE}
...
```{r missing_values_graph1, message=FALSE, warning=FALSE}
...
```{r missing_values_graph2, message=FALSE, warning=FALSE}
...
```{r plot_correlation_pearson_upper_quadrant, fig.width=6, fig.height=6, dpi=150}
...
```{r plot_correlation_pearson_cross, fig.width=6, fig.height=6, dpi=150}
...
```{r plot_correlation_pearson_lower_quadrant, fig.width=6, fig.height=6, dpi=150}
...
```{r pair_plot_correlated_variables, warning=FALSE}
...
```{r outliers_detection, warning=FALSE}
...
# Graficar los valores atípicos con un boxplot horizontal
# Se muestra el numero total de valores atípicos
ggplot(data_long, aes(x = Valores, y = Variable)) +
  geom_boxplot(fill = "skyblue", color = "darkblue") +
  geom_jitter(aes(color = abs(Valores) > 3), width = 0.1) +
  scale_color_manual(values = c("black", "red")) +
  labs(title = "Detección de Valores Atípicos en Variables Predictoras",
...
### Regresión lineal robusta de Huber: radio del exoplaneta según la profundidad del tránsito (`pl_trandep`)
...
```{r univar_reg_pl_trandep_huber_plot, warning=FALSE}
...
ggplot(kepler_data_num, aes(x = pl_trandep, y = pl_radj)) +
  ...
  labs(title = "Regresión de Huber: Radio del Exoplaneta segun Profundidad del Tránsito",
       x = "Profundidad del Tránsito (%)",
       y = "Radio del Exoplaneta (Júpiter)") +
  theme_minimal()
...
El modelo de regresión de Huber muestra una relación significativa entre **`pl_trandep`** y **`pl_radj`**, con un coeficiente de **0.6289**. Cada aumento unitario en **`pl_trandep`** se asocia con un incremento de **0.6289 en `pl_radj`**. Además, el modelo tiene un **error estándar de los residuos de 0.3876**, lo que indica una variabilidad moderada en las predicciones.

Ambos coeficientes son altamente significativos (t > 2 por mucho), lo que sugiere que hay una relación clara y fuerte entre la profundidad del tránsito y el radio del planeta. Sin embargo, de los datos originales, solo 41 observaciones fueron usadas y 2505 observaciones fueron descartadas. Eso puede pasar si faltan datos en **`pl_radj`** o **`pl_trandep`** o se perdieron en el preprocesamiento (posiblemente el tratamiento del método robusto) los dejó fuera.
...
```{r univar_reg_pl_rvamp_log_transf_plot, warning=FALSE}
...
```{r semi_supervised_kmeans}

library(dplyr)
library(lightgbm)
library(Matrix)

# asignar un id numerico único a cada fila
kepler_data <- kepler_data %>%
  mutate(id = row_number())

# Separar PHI y features
# Filtrar columnas numéricas **con solo valores finitos**
...
```{r semi_supervised_kmeans_pca_plot}

library(ggplot2)

# 1. Asegurarse de que features está estandarizado (como se usó en K-means)
...
ggplot(pca_data, aes(x = PC1, y = PC2, color = cluster)) +
  ...
  labs(
    ...
    subtitle = "Los triángulos negros indican planetas con PHI conocido",
...
for (cluster_id in unique(clusters)) {
  ...
  if (sum(labeled) > 0) {
    ...
    # buscar cluster más cercano con PHI
...
# Crear pesos: más alto para muestras reales
weights <- ifelse(is.na(phi), 1, 10)

```

Tercero se entrena el modelo LightGBM utilizando los clusters como etiquetas débiles y los pesos para ajustar la importancia de las muestras reales frente a las no etiquetadas.

```{r semi_supervised_lightgbm}    
...
```{r semi_supervised_lightgbm_importance}
...
```{r semi_supervised_habitable_planets}}
...
Para mejorar el modelo de regresión logística, es fundamental aplicar técnicas de regularización (como L1 o L2) para mitigar el sobreajuste derivado del escaso número de observaciones. También se recomienda una selección de variables cuidadosa, priorizando aquellas con fuerte correlación con PHI o justificación física. La inclusión de transformaciones no lineales e interacciones puede capturar relaciones más complejas, y el uso de validación cruzada tipo leave-one-out (LOOCV) maximiza el aprovechamiento del conjunto limitado de datos. Alternativamente, una regresión logística bayesiana permitiría incorporar incertidumbre y conocimiento previo sobre los factores que influyen en la habitabilidad.

Para el modelo semi-supervisado, las mejoras se enfocan en refinar la generación de pseudo-etiquetas y aprovechar mejor la estructura del conjunto de datos. Una opción es utilizar pseudo-labeling iterativo, donde el modelo actualiza sus predicciones en ciclos sucesivos. También puede explorarse un ajuste dinámico del número de clusters o el uso de algoritmos de clustering más flexibles. Incorporar embeddings generados por autoencoders o técnicas no supervisadas permitiría representar mejor las relaciones latentes entre planetas. Además, asignar pesos variables a las pseudo-etiquetas según su confiabilidad y validar únicamente sobre los casos con PHI real mejora la robustez y la capacidad de generalización del modelo.
