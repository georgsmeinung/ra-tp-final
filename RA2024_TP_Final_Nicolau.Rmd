...
# Crear un gráfico Q-Q de los residuos
qqnorm(res_pl_trandep_poly, main = "Gráfico Q-Q de los Residuos")
...
kepler_data_logistic_normalized <- kepler_data_logistic_normalized %>%
  select(-pl_eqt, -pl_insol, -pl_orbsmax)
...
Una opción para mejorar el modelo es regresionar las variables faltantes para calcular el PHI de más exoplanetas. Se pueden usar las variables de características de la estrella y del sistema planetario para predecir las variables faltantes. Se regresionan las variables `pl_eqt`, `pl_insol` y `pl_orbsmax` a partir de las variables de características de la estrella y del sistema planetario.

``` {r most_correlated_variables_pl_eqt, include=FALSE}

# Seleccionar de la matriz de correlación las variables con correlacion mayor o
# igual a 0.5 en valor absoluto con la variable faltante
correlated_vars_pl_eqt <- names(which(abs(correlation_matrix_pearson["pl_eqt", ]) >= 0.5))

# Se genera un dataframe con la correlacion entre las variables seleccionadas
# en correlated_vars y la variable faltante tomando de la matriz de correlacion
# solo las filas y columnas correspondientes a las variables seleccionadas
correlation_pl_eqt_df <- data.frame(
  Variable = names(correlation_matrix_pearson[correlated_vars_pl_eqt, "pl_eqt"]),
  Correlacion = correlation_matrix_pearson[correlated_vars_pl_eqt, "pl_eqt"]
)

# Descartar las otras columnas en la ecuacion de PHI
# pl_eqt, pl_insol y pl_orbsmax
correlation_pl_eqt_df <- correlation_pl_eqt_df[!correlation_pl_eqt_df$Variable %in% c("pl_eqt", "pl_insol", "pl_orbsmax"), ]

# Contar la cantidad de faltantes de cada variable
# y agregar a la tabla
correlation_pl_eqt_df <- cbind(correlation_pl_eqt_df, Faltantes = colSums(is.na(kepler_data_num[, correlation_pl_eqt_df$Variable])))


# Ordenar por correlacion descendente
correlation_pl_eqt_df <- correlation_pl_eqt_df[order(-abs(correlation_pl_eqt_df$Correlacion)), ]

# Eliminar la columna de indice
rownames(correlation_pl_eqt_df) <- NULL

# Mostrar tabla formateada
kable(correlation_pl_eqt_df, columns = c("Variable","Correlación","Faltantes"), align = "l")

```

Por tener la menor cantidad de faltantes y correlación con la variable `pl_eqt` se elige la variable `pl_trandur` (duracion del tránsito) como predictor de `pl_eqt` para imputar los valores faltantes de `pl_eqt`.

``` {r pl_eqt_reg, include=FALSE}

# Regresion de pl_eqt con pl_trandur
model_pl_eqt <- lm(pl_eqt ~ pl_trandur, data = kepler_data_num)
...
``` {r pl_eqt_reg_plot, include=FALSE}

# Scatter plot de pl_eqt vs pl_trandur con la recta de regresión en rojo
# y los margenes de confianza en gris
ggplot(kepler_data_num, aes(x = pl_trandur, y = pl_eqt)) +
  geom_point(color = "steelblue") +
  geom_smooth(method = "lm", color = "indianred", fill = "grey", alpha = 0.2) +
  labs(title = "Regresión Lineal: Temperatura de Equilibrio vs Duración del Tránsito",
       x = "Duración del Tránsito (días)",
       y = "Temperatura de Equilibrio (K)") +
  theme_minimal()

```
Los datos ajustados se iran usando para imputar los valores faltantes de `pl_eqt` en el data frame `kepler_data_num_imputed` que se va a usar también para imputar los valores faltantes de `pl_insol` y `pl_orbsmax`.

``` {r pl_eqt_imputation, include=FALSE}

# Copiar los datos en nuevo data frame
kepler_data_num_imputed <- kepler_data_num

# Imputar los valores faltantes de pl_eqt con los datos ajustados
kepler_data_num_imputed$pl_eqt[is.na(kepler_data_num_imputed$pl_eqt)] <- predict(model_pl_eqt, newdata = kepler_data_num_imputed[is.na(kepler_data_num_imputed$pl_eqt), ])

```

Luego se regresiona los datos faltantes de `pl_insol` a partir de las variables de características de la estrella y del sistema planetario.

``` {r most_correlated_variables_pl_insol, include=FALSE}

# Seleccionar de la matriz de correlación las variables con correlacion mayor o
# igual a 0.5 en valor absoluto con la variable faltante
correlated_vars_pl_insol <- names(which(abs(correlation_matrix_pearson["pl_insol", ]) >= 0.5))

# Se genera un dataframe con la correlacion entre las variables seleccionadas
# en correlated_vars y la variable faltante tomando de la matriz de correlacion
# solo las filas y columnas correspondientes a las variables seleccionadas
correlation_pl_insol_df <- data.frame(
  Variable = names(correlation_matrix_pearson[correlated_vars_pl_insol, "pl_insol"]),
  Correlacion = correlation_matrix_pearson[correlated_vars_pl_insol, "pl_insol"]
)

# Descartar las otras columnas en la ecuacion de PHI
# pl_eqt, pl_insol y pl_orbsmax
correlation_pl_insol_df <- correlation_pl_insol_df[!correlation_pl_insol_df$Variable %in% c("pl_eqt", "pl_insol", "pl_orbsmax"), ]

# Contar la cantidad de faltantes de cada variable
# y agregar a la tabla
correlation_pl_insol_df <- cbind(correlation_pl_insol_df, Faltantes = colSums(is.na(kepler_data_num[, correlation_pl_insol_df$Variable])))

# Ordenar por correlacion descendente
correlation_pl_insol_df <- correlation_pl_insol_df[order(-abs(correlation_pl_insol_df$Correlacion)), ]

# Eliminar la columna de indice
rownames(correlation_pl_insol_df) <- NULL

# Mostrar tabla formateada
kable(correlation_pl_insol_df, columns = c("Variable","Correlación","Faltantes"), align = "l")

``` 

Por tener la menor cantidad de faltantes y correlación con la variable `pl_insol` se elige la variable `pl_orbincl` (inclinación orbital) como predictor de `pl_insol` para imputar los valores faltantes de `pl_insol` dado que se utilizar la variable `pl_trandur` para imputar los valores faltantes de `pl_eqt`.

``` {r pl_insol_reg, include=FALSE}

# Regresion de pl_insol con pl_orbincl
model_pl_insol <- lm(pl_insol ~ pl_orbincl, data = kepler_data_num)

model_pl_insol_summary <- summary(model_pl_insol)

model_pl_insol_summary

```
``` {r pl_insol_reg_plot, include=FALSE}

# Scatter plot de pl_insol vs pl_trandur con la recta de regresión en rojo
# y los margenes de confianza en gris
ggplot(kepler_data_num, aes(x = pl_orbincl, y = pl_insol)) +
  geom_point(color = "steelblue") +
  geom_smooth(method = "lm", color = "indianred", fill = "grey", alpha = 0.2) +
  labs(title = "Regresión Lineal: Insolación vs Duración del Tránsito",
       x = "Inclinación Orbital (°)",
       y = "Insolación (S)") +
  theme_minimal()

```
Los datos ajustados se iran usando para imputar los valores faltantes de `pl_insol` en el data frame `kepler_data_num_imputed`.

``` {r pl_insol_imputation, include=FALSE}

# Imputar los valores faltantes de pl_insol con los datos ajustados
kepler_data_num_imputed$pl_insol[is.na(kepler_data_num_imputed$pl_insol)] <- predict(model_pl_insol, newdata = kepler_data_num_imputed[is.na(kepler_data_num_imputed$pl_insol), ])

```

``` {r most_correlated_variables_pl_orbsmax, include=FALSE}

# Seleccionar de la matriz de correlación las variables con correlacion mayor o
# igual a 0.5 en valor absoluto con la variable faltante
correlated_vars_pl_orbsmax <- names(which(abs(correlation_matrix_pearson["pl_orbsmax", ]) >= 0.5))

# Se genera un dataframe con la correlacion entre las variables seleccionadas
# en correlated_vars y la variable faltante tomando de la matriz de correlacion
# solo las filas y columnas correspondientes a las variables seleccionadas
correlation_pl_orbsmax_df <- data.frame(
  Variable = names(correlation_matrix_pearson[correlated_vars_pl_orbsmax, "pl_orbsmax"]),
  Correlacion = correlation_matrix_pearson[correlated_vars_pl_orbsmax, "pl_orbsmax"]
)

# Descartar las otras columnas en la ecuacion de PHI
# pl_eqt, pl_insol y pl_orbsmax
correlation_pl_orbsmax_df <- correlation_pl_orbsmax_df[!correlation_pl_orbsmax_df$Variable %in% c("pl_eqt", "pl_insol", "pl_orbsmax"), ]

# Contar la cantidad de faltantes de cada variable
# y agregar a la tabla
correlation_pl_orbsmax_df <- cbind(correlation_pl_orbsmax_df, Faltantes = colSums(is.na(kepler_data_num[, correlation_pl_orbsmax_df$Variable])))

# Ordenar por correlacion descendente
correlation_pl_orbsmax_df <- correlation_pl_orbsmax_df[order(-abs(correlation_pl_orbsmax_df$Correlacion)), ]

# Eliminar la columna de indice
rownames(correlation_pl_orbsmax_df) <- NULL

# Mostrar tabla formateada
kable(correlation_pl_orbsmax_df, columns = c("Variable","Correlación","Faltantes"), align = "l")

``` 

Por tener la menor cantidad de faltantes y correlación con la variable `pl_eqt` se elige la variable `pl_orbper` (periodo orbital) como predictor de `pl_orbsmax` para imputar los valores faltantes de `pl_orbsmax`.

``` {r pl_orbsmax_reg, include=FALSE}

# Regresion de pl_orbsmax con pl_orbper
model_pl_orbsmax <- lm(pl_orbsmax ~ pl_orbper, data = kepler_data_num)
...
``` {r pl_orbsmax_reg_plot, include=FALSE}

# Scatter plot de pl_orbsmax vs pl_orbper con la recta de regresión en rojo
# y los margenes de confianza en gris
ggplot(kepler_data_num, aes(x = pl_orbper, y = pl_orbsmax)) +
  geom_point(color = "steelblue") +
  geom_smooth(method = "lm", color = "indianred", fill = "grey", alpha = 0.2) +
  labs(title = "Regresión Lineal: Semieje Mayor Orbital vs Periodo Orbital",
...
Los datos ajustados se iran usando para imputar los valores faltantes de `pl_orbsmax` se almacenan en el data frame `kepler_data_num_imputed`.

``` {r pl_orbsmax_imputation, include=FALSE}

# Imputar los valores faltantes de pl_orbsmax con los datos ajustados
kepler_data_num_imputed$pl_orbsmax[is.na(kepler_data_num_imputed$pl_orbsmax)] <- predict(model_pl_orbsmax, newdata = kepler_data_num_imputed[is.na(kepler_data_num_imputed$pl_orbsmax), ])

```

Finalmente se calcula el índice de habitabilidad planetaria (PHI) con las variables imputadas y se ajusta un modelo de regresión logística para predecir la habitabilidad de los exoplanetas.

``` {r log_reg_imputed}

# Crear un data frame con las variables seleccionadas
kepler_data_logistic_imputed <- kepler_data_num_imputed %>%
  dplyr::select(pl_eqt, pl_insol, pl_orbsmax)

# Normalizar las variables con los límites definidos
kepler_data_logistic_imputed <- kepler_data_logistic_imputed %>%
  mutate(
    pl_eqt_normalized = pmin(1, pmax(0, (pl_eqt - 200) / (400 - 200))),
    pl_insol_normalized = pmin(1, pmax(0, (pl_insol - 0.3) / (1.7 - 0.3))),
    pl_orbsmax_normalized = pmin(1, pmax(0, (pl_orbsmax - 1.05) / (2 - 1.05)))
...
kepler_data_logistic_imputed <- kepler_data_logistic_imputed %>%
  mutate(
    phi = 0.4 * pl_eqt + 0.3 * pl_insol + 0.3 * pl_orbsmax 
  )
...
# Crear un modelo de regresión logística
model_logistic_imputed <- glm(phi ~ ., data = kepler_data_logistic_imputed, family = binomial)

model_logistic_imputed_summary <- summary(model_logistic_imputed)

model_logistic_imputed_summary
