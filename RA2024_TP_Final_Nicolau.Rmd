...
```{r univariate_regression_analysis_pl_radj_pl_trandep}

# Regresion de pl_radj con pl_trandep
model_pl_radj_pl_trandep <- lm(pl_radj ~ pl_trandep, data = kepler_data_num)

model_pl_radj_pl_trandep_summary <- summary(model_pl_radj_pl_trandep)

model_pl_radj_pl_trandep_summary
```

```{r univariate_regression_analysis_pl_radj_pl_trandep_plot, warning=FALSE}
...
```{r univariate_regression_analysis_pl_radj_pl_trandep_residuals_plot, warning=FALSE}

# Obtener los residuos y valores ajustados
residuals <- residuals(model_pl_radj_pl_trandep)

fitted_values <- fitted(model_pl_radj_pl_trandep)
...
```{r univariate_regression_analysis_pl_radj_pl_trandep_qq_plot, warning=FALSE}
...
```{r univariate_regression_analysis_pl_radj_pl_trandep_shapiro_test}
...
```{r univariate_regression_analysis_pl_radj_pl_trandep_bp_test}

# Prueba de homocedasticidad de Breusch-Pagan
library(lmtest)

# Prueba de Breusch-Pagan
bp_test <- bptest(model_pl_radj_pl_trandep)

bp_test

```

```{r univariate_regression_analysis_pl_radj_pl_ratdor}

# Regresion de pl_radj con st_teff
model_pl_radj_pl_ratdor <- lm(pl_radj ~ pl_ratdor, data = kepler_data_num)

model_pl_radj_pl_ratdor_summary <- summary(model_pl_radj_pl_ratdor)

model_pl_radj_pl_ratdor_summary
```

```{r univariate_regression_analysis_pl_radj_pl_ratdor_plot, warning=FALSE}
...
```{r univariate_regression_analysis_pl_radj_pl_ratdor_residuals_plot, warning=FALSE}

# Obtener los residuos y valores ajustados
residuals_ratdor <- residuals(model_pl_radj_pl_ratdor)

fitted_values_ratdor <- fitted(model_pl_radj_pl_ratdor)

# Crear un data frame con los residuos y valores ajustados
residuals_ratdor_df <- data.frame(
  Residuals = residuals_ratdor,
  Fitted_Values = fitted_values_ratdor
)

# Scatter plot de residuos vs valores ajustados
ggplot(residuals_ratdor_df, aes(x = Fitted_Values, y = Residuals)) +
  geom_point(color = "steelblue") +
  geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  labs(title = "Residuos vs Valores Ajustados",
       x = "Valores Ajustados",
...
```{r univariate_regression_analysis_pl_radj_pl_ratdor_qq_plot, warning=FALSE}

# Crear un gráfico Q-Q de los residuos
qqnorm(residuals_ratdor, main = "Gráfico Q-Q de los Residuos")

```

```{r univariate_regression_analysis_pl_radj_pl_ratdor_shapiro_test}

# Prueba de normalidad de Shapiro-Wilk
shapiro_test_ratdor <- shapiro.test(residuals_ratdor)

shapiro_test_ratdor

```

```{r univariate_regression_analysis_pl_radj_pl_ratdor_bp_test}

# Prueba de homocedasticidad de Breusch-Pagan
library(lmtest)

# Prueba de Breusch-Pagan
bp_test_ratdor <- bptest(model_pl_radj_pl_ratdor)

bp_test_ratdor

```

```{r univariate_regression_analysis_pl_radj_pl_rvamp}

# Regresion de pl_radj con pl_rvamp
model_pl_radj_pl_rvamp <- lm(pl_radj ~ pl_rvamp, data = kepler_data_num)

model_pl_radj_pl_rvamp_summary <- summary(model_pl_radj_pl_rvamp)

model_pl_radj_pl_rvamp_summary
```

```{r univariate_regression_analysis_pl_radj_pl_rvamp_plot, warning=FALSE}
...
```{r univariate_regression_analysis_pl_radj_pl_rvamp_residuals_plot, warning=FALSE}

# Obtener los residuos y valores ajustados
residuals_rvamp <- residuals(model_pl_radj_pl_rvamp)

fitted_values_rvamp <- fitted(model_pl_radj_pl_rvamp)

# Crear un data frame con los residuos y valores ajustados
residuals_rvamp_df <- data.frame(
  Residuals = residuals_rvamp,
  Fitted_Values = fitted_values_rvamp
)

# Scatter plot de residuos vs valores ajustados
ggplot(residuals_rvamp_df, aes(x = Fitted_Values, y = Residuals)) +
  geom_point(color = "steelblue") +
  geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  labs(title = "Residuos vs Valores Ajustados",
       x = "Valores Ajustados",
...
```{r univariate_regression_analysis_pl_radj_pl_rvamp_qq_plot, warning=FALSE}

# Crear un gráfico Q-Q de los residuos
qqnorm(residuals_rvamp, main = "Gráfico Q-Q de los Residuos")

```

```{r univariate_regression_analysis_pl_radj_pl_rvamp_shapiro_test}

# Prueba de normalidad de Shapiro-Wilk
shapiro_test_rvamp <- shapiro.test(residuals_rvamp)

shapiro_test_rvamp

```

```{r univariate_regression_analysis_pl_radj_pl_rvamp_bp_test}

# Prueba de homocedasticidad de Breusch-Pagan
library(lmtest)

# Prueba de Breusch-Pagan
bp_test_rvamp <- bptest(model_pl_radj_pl_rvamp)

bp_test_rvamp

```

```{r univariate_regression_analysis_pl_radj_st_rad}

# Regresion de pl_radj con st_rad

model_pl_radj_st_rad <- lm(pl_radj ~ st_rad, data = kepler_data_num)

model_pl_radj_st_rad_summary <- summary(model_pl_radj_st_rad)

model_pl_radj_st_rad_summary
```

```{r univariate_regression_analysis_pl_radj_st_rad_plot, warning=FALSE}
...
```{r univariate_regression_analysis_pl_radj_st_rad_residuals_plot, warning=FALSE}

# Obtener los residuos y valores ajustados

residuals_st_rad <- residuals(model_pl_radj_st_rad)

fitted_values_st_rad <- fitted(model_pl_radj_st_rad)

# Crear un data frame con los residuos y valores ajustados

residuals_st_rad_df <- data.frame(
  Residuals = residuals_st_rad,
  Fitted_Values = fitted_values_st_rad
)

# Scatter plot de residuos vs valores ajustados
ggplot(residuals_st_rad_df, aes(x = Fitted_Values, y = Residuals)) +
  geom_point(color = "steelblue") +
  geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  labs(title = "Residuos vs Valores Ajustados",
       x = "Valores Ajustados",
...
```{r univariate_regression_analysis_pl_radj_st_rad_qq_plot, warning=FALSE}

# Crear un gráfico Q-Q de los residuos

qqnorm(residuals_st_rad, main = "Gráfico Q-Q de los Residuos")

```

```{r univariate_regression_analysis_pl_radj_st_rad_shapiro_test}

# Prueba de normalidad de Shapiro-Wilk
#shapiro_test_st_rad <- shapiro.test(residuals_st_rad)

#shapiro_test_st_rad

```

```{r univariate_regression_analysis_pl_radj_st_rad_bp_test}

# Prueba de homocedasticidad de Breusch-Pagan
library(lmtest)

# Prueba de Breusch-Pagan
bp_test_st_rad <- bptest(model_pl_radj_st_rad)
...
model_multivariate <- lm(pl_radj ~ pl_trandep + pl_ratdor + pl_rvamp + st_rad, data = kepler_data_num)
...
```{r multivariate_regression_analysis_residuals_plot, warning=FALSE}

# Obtener los residuos y valores ajustados
residuals_multivariate <- residuals(model_multivariate)

fitted_values_multivariate <- fitted(model_multivariate)

# Crear un data frame con los residuos y valores ajustados
residuals_multivariate_df <- data.frame(
  Residuals = residuals_multivariate,
  Fitted_Values = fitted_values_multivariate
)

# Scatter plot de residuos vs valores ajustados
ggplot(residuals_multivariate_df, aes(x = Fitted_Values, y = Residuals)) +
  geom_point(color = "steelblue") +
  geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  labs(title = "Residuos vs Valores Ajustados",
       x = "Valores Ajustados",
...
```{r multivariate_regression_analysis_qq_plot, warning=FALSE}

# Crear un gráfico Q-Q de los residuos
qqnorm(residuals_multivariate, main = "Gráfico Q-Q de los Residuos")

```

```{r multivariate_regression_analysis_shapiro_test}

# Prueba de normalidad de Shapiro-Wilk
shapiro_test_multivariate <- shapiro.test(residuals_multivariate)

shapiro_test_multivariate

```

```{r multivariate_regression_analysis_bp_test}

# Prueba de homocedasticidad de Breusch-Pagan
library(lmtest)

# Prueba de Breusch-Pagan
bp_test_multivariate <- bptest(model_multivariate)

bp_test_multivariate
...
La insolación es un factor crítico para mantener agua líquida en la superficie del planeta, pero su impacto depende de otros factores como la atmósfera y el albedo, según la literatura especializada el peso de la insolación en el índice de habitabilidad es de $w_2=0.30$. 
Es importante destacar que El rango de $I_{min}$ e $I_{max}$ puede variar según la luminosidad de la estrella, ya que la zona habitable cambia en función del tipo estelar por lo que esta estimación sirve para estrellas similares al Sol.
...
$$
En la mayoría de los modelos, el peso para el valor normalizado de `pl_orbsmax` oscila entre 15% y 25%. Esto se debe a que `pl_orbsmax` complementa indicadores más directos como 
`pl_eqt` (temperatura de equilibrio) y `pl_insol` (insolación). Se tomará un peso de $w_3=0.20$.
...
Donde:
- f5(x) es la normalización de la temperatura efectiva de la estrella (`st_teff`) con la normalización de la temperatura efectiva de la estrella ideal para la habitabilidad en el rango de 4000K a 6000K.
- f6(x) es la normalización de la masa de la estrella (`st_mass`) con la normalización de la masa de la estrella ideal para la habitabilidad en el rango de 0.8 a 1.2 masas solares.
- f7(x) es la normalización de la luminosidad de la estrella (`st_lum`) con la normalización de la luminosidad de la estrella ideal para la habitabilidad en el rango de -0.5 (estrella demasiado débil) a 0.5 (estrella con radiación excesiva) veces la luminosidad solar.
- f8(x) es la normalización de la gravedad superficial de la estrella (`st_logg`) con la normalización de la gravedad superficial de la estrella ideal para la habitabilidad en el rango de 4.0 (estrella en evolución fuera de la secuencia principal) a 4.5 (condiciones poco comunes para estrellas típicas).
- f9(x) es la normalización de la edad de la estrella (`st_age`) con la normalización de la edad de la estrella ideal para la habitabilidad en el rango de 1 a 10 gigaaños.
...
```{r logistic_regression_analysis_confusion_matrix}

# Dividir los datos en conjunto de entrenamiento y prueba
train_indices <- sample(1:nrow(kepler_data_logistic_normalized), size = 0.8 * nrow(kepler_data_logistic_normalized))
train_data <- kepler_data_logistic_normalized[train_indices, ]
test_data <- kepler_data_logistic_normalized[-train_indices, ]

# Ajustar el modelo nuevamente usando los datos de entrenamiento
model <- glm(phi ~ ., family = binomial, data = train_data)
