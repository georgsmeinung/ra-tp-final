...
resumen <- skim(transactions_data_num) 
...
```{r sample_missing_values, echo=FALSE}
...
# Seleccionar solo columnas id, date, amount, merchant_id, merchant_city
missing_state_data <- missing_state_data %>%
  select(id, date, amount, merchant_id, merchant_city)

# Mostrar una muestra de las transacciones con valores faltantes en merchant_state
kable(head(missing_state_data))
...
# Seleccionar solo columnas id, date, amount, merchant_id, merchant_city, merchant_state y zip
missing_zip_data <- missing_zip_data %>%
  select(id, date, amount, merchant_id, merchant_city, merchant_state, zip)

# Mostrar una muestra de las transacciones con valores faltantes en zip
kable(head(missing_zip_data))
...
kable(paste("Paises encontrados: ",length(unique_countries$merchant_state)))
...
```{r plot_amount_distribution, echo=FALSE, warning=FALSE}
...
# Seleccionar las columnas numéricas de los datos de transacciones y la primera
# mitad de los que comienzan con st_ además de las 7 primeras columnas 
# del dataset sin prefijo (las de originales)
...
corrplot(
  ...
  tl.col = "black",
  col = colorRampPalette(c("salmon", "white", "skyblue"))(10),
  title = "Matriz de Correlación - Pirámide Demográfica"
)
